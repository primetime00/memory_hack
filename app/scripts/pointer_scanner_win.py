...
class PointerScanner(BaseScript):
    ...
    def build_ui(self):
        ...
        self.add_ui_element(Input("ADDRESS", "Address", on_changed=self.ctrl_changed, change_on_focus=False,
                                  default_text="", children=[Button("PASTE", "Paste", on_pressed=self.ctrl_pressed)]))
        self.add_ui_element(Input("OFFSET", "Offset", on_changed=self.ctrl_changed, change_on_focus=False,
                                  default_text="4096"))
        self.add_ui_element(
            Select("LEVELS", "Levels", on_changed=self.ctrl_changed, values=[(str(x), str(x)) for x in range(1, 7)]))
        self.add_ui_element(MultiSelect("REGIONS", "Regions", on_changed=self.ctrl_changed,
                                        values=[(str(x), str(x)) for x in range(1, 7)]))
    ...
    def ctrl_changed(self, ele: BaseUI, value):
        if ele.get_name() == 'ADDRESS' or ele.get_name() == 'OFFSET' or ele.get_name() == 'LEVELS':
            ...
            if value != '_null':
                ...
                self.put_data('SEARCHER',
                              SearcherMulti(self.get_memory(), directory=scripts_memory_directory, write_only=True, results=SearchResults(db_path=scripts_memory_directory.joinpath('pscan.db'))))
                self.find_regions()
                self.check_for_start()
            else:
    ...
    def search_break(self):
        ...
        self.get_ui_control("SEARCH").show()
        [self.get_ui_control(ctrl_name).enable() for ctrl_name in
         ['PROCS', 'REFRESH', 'ADDRESS', 'OFFSET', 'LEVELS', 'REGIONS']]
    ...
    def prepare_search(self):
        ...
        [self.get_ui_control(ctrl_name).disable() for ctrl_name in
         ['PROCS', 'REFRESH', 'ADDRESS', 'OFFSET', 'LEVELS', 'REGIONS']]
    ...
    def perform_search(self, address, offset, levels):
        ...
        node_bounds = [(x['start'], x['stop']) for x in proc_map if len(x['pathname']) > 1]
                    ...
                    for index in result_indexes:
                        ...
                        if r['address'] not in address_set:
                            ...
                            item = {'address': r['address'],
                                    'offset': address - int.from_bytes(r['value'], byteorder="little", signed=False),
                                    'children': []}
                            result_counter += 1
                        ...
                        if poll_timer.has_elapsed():
                            self.get_ui_control("STATUS").set_text(
                                'Search level {}<br>Found {} possible pointers.<br>{} potential static pointers.'.format(
                                    i, result_counter, static_counter))
                    ...
                    for p in lvl_map[i - 1]:
                        ...
                        if poll_timer.has_elapsed():
                            self.get_ui_control("STATUS").set_text(
                                'Search level {}<br>Found {} possible pointers.<br>{} potential static pointers.'.format(
                                    i, result_counter, static_counter))
                            print("Found {} result and {} static {} invalid {} reused {} zero searches".format(
                                result_counter, static_counter, invalid_counter, reuse_counter, zero_counter))
                            print("Number of searches {} / Average search time: {} / Searches per minute: {}".format(
                                number_of_searches, total_search_time / number_of_searches,
                                60 * number_of_searches / (time.time() - start_time)))
                        p['children'] = nx
    ...
    def organize(self, pointers, holder):
        for pointer in pointers:
            if pointer['children']:
                ...
                if pathname != ' ':
                    holder.append(
                        {'address': pointer['address'], 'path': pathname, 'node': node, 'base_offset': base_offset,
